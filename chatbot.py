...
# Financial Chatbot - Optimized with Riskfolio-Lib, Technical Scan, Risk, Multi-Source News Sentiment + ETS Forecasting
...
from statsmodels.tsa.holtwinters import ExponentialSmoothing # <--- Added for ETS
import scipy.stats as stats
import sklearn.covariance as skcov
from sklearn.metrics import mean_squared_error # <--- Added for ETS evaluation
...
# SHAREPREDICTIONS_API_KEY = 'fX0NTtYD.6DQcbxP4f4tzkemzMM0romuSk8iOsNTx' # Removed SharePredictions Key
# --- End of Hardcoded API Keys ---

# --- NEWS SENTIMENT Configuration ---
NEWS_API_ENDPOINT = 'https://newsapi.org/v2/everything'
FMP_ENDPOINT = 'https://financialmodelingprep.com/api'
# RSS FEEDS - Add more relevant feeds if desired
RSS_FEEDS = [
...
try:
    ...
    def _fallback_SemiDeviation(X):
        a = np.array(X, ndmin=1).flatten(); mu = np.mean(a); diff = a - mu; downside_diff = diff[diff < 0]; variance = np.sum(downside_diff**2) / max(1, len(a) - 1); return np.sqrt(variance)
    def _fallback_CDaR_Abs(X, alpha=0.01):
        a = np.array(X, ndmin=1).flatten(); prices = np.insert(a, 0, 0); NAV = np.cumsum(prices) + 1; DD = []; peak = -np.inf;
        for i in NAV: peak = max(peak, i); DD.append(peak - i)
        sorted_DD = np.sort(np.array(DD)); index = int(np.ceil((1 - alpha) * len(sorted_DD)) - 1); return sorted_DD[max(0, min(index, len(sorted_DD) - 1))]
...
# Updated Title
st.set_page_config(page_title="📈 Financial Chat + Risk + Signals + News + Forecasting", layout="wide", initial_sidebar_state="expanded")
...
# Removed SharePredictions check
...
if missing_keys:
    ...
    # Only stop if critical key (OpenAI) is missing
    if "OpenAI API Key" in missing_keys:
         st.stop()
    else:
         # Allow running with warnings for optional keys
...
def parse_date(date_string):
    """Attempts to parse various date formats into ISO format."""
    if not date_string: return None
    try:
        dt = date_parser.parse(date_string)
        # Standardize to ISO format with 'Z' for UTC (or offset if available)
        return dt.isoformat().split('+')[0].split('.')[0] + "Z"
    except (ValueError, TypeError, OverflowError):
        logging.debug(f"Could not parse date: {date_string}")
        return None

@st.cache_data(ttl=300) # Cache Yahoo summary data for 5 minutes
def get_stock_data(ticker):
    """Fetches summary stock data from Yahoo Finance using yfinance."""
    ...
    try:
        ...
        if not info or not info.get('symbol'):
            ...
            if hist.empty: logging.warning(f"Ticker {ticker} not found or invalid on Yahoo Finance."); st.toast(f"⚠️ Couldn't find Yahoo equity data for '{ticker}'.", icon="⚠️"); return None
                 ...
                 # Use history close if price fields are None
        ...
        if quote_type not in ['EQUITY', 'ETF']:
             ...
             # Only warn/toast if it's not one of the expected types
             if quote_type not in ['N/A', 'Undefined']:
                 ...
                 # Allow processing to continue, but forecasting might fail or be inaccurate.
                 # return None # Optionally return None to stop further processing for unsupported types

        num_opinions = info.get("numberOfAnalystOpinions")
        data = {
            "ticker": info.get("symbol", yf_ticker).upper(),
            "companyName": info.get("shortName", info.get("longName", "N/A")),
            "sector": info.get("sector", "N/A"),
            "industry": info.get("industry", "N/A"),
            "quoteType": quote_type, # Keep quote type
            "priceForDisplay": info.get("currentPrice", info.get("regularMarketPrice", info.get("regularMarketOpen", info.get("previousClose")))),
            ...
            "numberOfAnalystOpinions": int(num_opinions) if num_opinions is not None and isinstance(num_opinions, (int, float)) and num_opinions > 0 else None,
...
def get_unified_yfinance_history(ticker: str, period="3y"): # Default to 3y for ETS
    """Fetches and caches historical daily data ONCE for all calculations (scan, risk, forecast)."""
    logging.info(f"Fetching UNIFIED yfinance history for {ticker}, period: {period}")
    try:
        yf_ticker_obj = yf.Ticker(ticker)
        # Use specified period directly, ETS needs longer history
        df = yf_ticker_obj.history(period=period, interval="1d", auto_adjust=False) # Use auto_adjust=False for raw OHLCV
        ...
        # Ensure enough data for calculations that need history
        min_rows_needed = 252 # Min for 1 year calculations (SMA200, Risk)
        if len(df) < min_rows_needed:
            logging.warning(f"Unified yfinance history for {ticker} has only {len(df)} rows (period={period}). Risk/Scan calculations might fail.")
            # Don't return None yet, maybe partial data is useful
        else:
            logging.info(f"Processed unified yfinance history for {ticker} ({len(df)} rows)")

        # Return Close prices separately for easy use in ETS, and full DF for others
        return df['Close'], df[required_cols]
...
def get_vix_cached():
    ...
    try:
        ...
        if current_vix is not None and isinstance(current_vix, (int, float)):
            logging.info(f"VIX from info: {current_vix:.2f}");
            ...
            if not hist.empty:
                 ...
                 if last_close is not None and isinstance(last_close, (int, float)):
                     logging.info(f"VIX from history: {last_close:.2f}");
...
# --- START: ETS FORECASTING FUNCTION ---
@st.cache_data(ttl=ETS_FORECAST_CACHE_DURATION_SECONDS)
def forecast_stock_ets_advanced(
    ticker: str,
    close_prices: pd.Series, # Accept pre-fetched close prices
    forecast_days: int = 7,
    volatility_window_recent: int = 21, # Approx 1 month
    volatility_window_long: int = 252, # Approx 1 year
    volatility_threshold: float = 1.5, # Recent std > 1.5 * long std triggers seasonality
    seasonal_period: int = 21, # Monthly seasonality (trading days)
    eval_test_size: int = 21 # Evaluate on last month
):
    """
    Generates a short-term price forecast using the Holt-Winters ETS model.
    Accepts pre-fetched close prices.
    """
    ...
    # --- Data Validation ---
    # Need enough data for long volatility window + returns + eval + min train
    ...
    try:
        ...
        if len(log_returns) >= volatility_window_long:
            ...
            # Avoid division by zero if long_std is 0
            if long_std > 1e-9 and (recent_std / long_std) > volatility_threshold:
                ...
                logger.info("[Forecast] Volatility within normal range, using non-seasonal model.")
        else:
            logger.warning("[Forecast] Not enough data for long-term volatility comparison, using non-seasonal model.")

    except Exception as e:
        logger.error(f"[Forecast] Error calculating volatility: {e}")
        logger.warning("[Forecast] Proceeding with non-seasonal model due to volatility calculation error.")
        use_seasonal = False # Default to non-seasonal on error

    # --- Model Setup ---
    model_params = {
        'trend': 'add', # Additive trend often suitable for prices
        'initialization_method': 'estimated', # Let statsmodels estimate starting values
        'use_boxcox': False, # Box-Cox usually not needed/helpful for prices unless variance strongly depends on level
        'damped_trend': True # Dampen the trend for longer forecasts - more conservative
    }
    if use_seasonal:
        model_params.update({
    ...
    # --- Train/Eval Split (Optional but Recommended) ---
    rmse = None
    evaluation_summary = "Evaluation: Not performed (insufficient data or error)."
    if len(close_prices) > eval_test_size + seasonal_period + 1: # Ensure enough data for split
        train_eval = close_prices[:-eval_test_size]
        test_eval = close_prices[-eval_test_size:]

        logger.info(f"[Forecast] Evaluating model on last {eval_test_size} days...")
        try:
            # Use 'N' frequency if available, otherwise None
            ...
            # Use context manager for potential optimization errors
            with warnings.catch_warnings():
                 warnings.simplefilter("ignore") # Suppress optimization warnings during eval fit
                 fitted_eval = eval_model.fit(optimized=True)
            ...
            evaluation_summary = f"Evaluation (last {eval_test_size} days): RMSE={rmse:.2f} (MAPE={mape:.2f}%)"
            logger.info(f"[Forecast] {evaluation_summary}")
        except Exception as e:
            logger.error(f"[Forecast] Error during evaluation fitting/forecasting: {e}")
            evaluation_summary = f"Evaluation Error: {e}"
            rmse = None # Reset RMSE if evaluation failed
    else:
         logger.warning("[Forecast] Not enough data to perform separate evaluation.")


    # --- Final Model Fit and Forecast ---
    ...
    try:
            ...
            try:
                 ...
                 if len(future_dates) == forecast_days:
                     forecast_result.index = future_dates
                      ...
                      logger.warning("[Forecast] Could not generate expected number of business dates for forecast index.")
            except Exception as date_err:
                 logger.warning(f"[Forecast] Error generating forecast date index: {date_err}. Using default numerical index.")
        else:
             logger.warning("[Forecast] Cannot generate date index for forecast (input index issue). Using default numerical index.")
        ...
        return forecast_result, evaluation_summary, model_used_desc # Return evaluation summary string
    except Exception as e:
        logger.error(f"[Forecast] Error during final model fitting/forecasting: {e}")
        return None, evaluation_summary, "Model Error" # Return evaluation summary even on final model error


# --- END: ETS FORECASTING FUNCTION ---


# --- START: MULTI-SOURCE NEWS SENTIMENT FUNCTIONS ---

# --- Helper: VADER Sentiment Analysis ---
def analyze_sentiment_vader(text):
    """Analyzes sentiment using VADER. Returns label and score."""
    if not text: return "Neutral", 0.0
    vs = vader_analyzer.polarity_scores(text) # Use the global analyzer instance
...
# --- Helper: Fetching Function - NewsAPI ---
def get_news_newsapi(ticker, api_key, days_back=NEWS_DAYS_BACK, page_size=NEWS_PAGE_SIZE):
    ...
    if not api_key or len(api_key) < 20:
        logging.warning("[News] NewsAPI key not set or seems invalid. Skipping NewsAPI.")
        return []
    to_date = datetime.now()
    from_date = to_date - timedelta(days=days_back)
    from_date_str = from_date.strftime('%Y-%m-%d')
    # Broaden query slightly - remove 'company' which might be too restrictive
    query = f'"{ticker}" AND (stock OR shares OR earnings OR market OR business)'
...
# --- Helper: Fetching Function - FMP ---
def get_news_fmp(ticker, api_key, limit=NEWS_FMP_LIMIT):
    """Fetches news articles from Financial Modeling Prep (FMP)."""
    logging.info(f"[News] Fetching news from Financial Modeling Prep for '{ticker}'...")
    if not api_key or len(api_key) < 20:
        logging.warning("[News] FMP API key not set or seems invalid. Skipping FMP.")
        return []
    ...
    try:
        ...
        if response.status_code != 200:
             ...
             else: response.raise_for_status() # Raise for other errors like 5xx
             return []
        data = response.json()
...
# --- Helper: Fetching Function - RSS ---
def get_news_rss(ticker, feed_urls, days_back=NEWS_DAYS_BACK):
    """Fetches and filters news from a list of RSS feeds."""
    ...
    for url_template in feed_urls:
        ...
        logging.debug(f"[News]   Parsing RSS feed: {url}") # Debug level for individual URLs
        try:
            headers = {'User-Agent': 'Mozilla/5.0 (compatible; FinancialBot/1.0; +http://example.com/bot)'}
            # Use etag and modified headers to potentially speed up feed parsing if unchanged
            feed_data = feedparser.parse(url, request_headers=headers, timeout=10) # Removed etag/modified for simplicity now
            ...
            for entry in feed_data.entries:
                ...
                # Filter by date *before* filtering by ticker for general feeds
                if published_dt_aware and published_dt_aware < cutoff_date:
                    continue

                # Filter by ticker relevance only *after* date filter
                # Check if the feed URL is ticker-specific or general
                is_ticker_specific_feed = '{ticker}' in url_template
                ticker_lower = ticker.lower(); title_lower = title.lower(); summary_lower = summary.lower()
                # Require ticker mention only for general feeds
                needs_ticker_mention = not is_ticker_specific_feed
                # Simple check for ticker symbol (e.g., "tsla", "(tsla)", "$tsla")
                ...
                # Include if it's a ticker-specific feed OR if it's a general feed that mentions the ticker
                ...
                elif needs_ticker_mention and not mentions_ticker:
                     logging.debug(f"[News]     RSS Skipping (General Feed, No Ticker Match): {title[:50]}...") # Log skipped general articles

        except Exception as e: logging.error(f"[News]     Error processing RSS feed {url}: {e}")
        time.sleep(0.2) # Be polite to RSS servers
...
# --- Main News Sentiment Orchestrator Function ---
...
def get_multi_source_news_sentiment(ticker: str, news_api_key: str, fmp_api_key: str):
    """
    Fetches news from NewsAPI, FMP, and RSS, performs VADER sentiment analysis,
    and returns an aggregated summary and counts.
    """
    logging.info(f"--- Starting Multi-Source News Sentiment Analysis for {ticker} ---"); start_time = time.time()
    # Fetch from all sources concurrently (or sequentially with sleeps)
    ...
    all_articles = newsapi_articles + fmp_articles + rss_articles
    logging.info(f"[News] Total articles fetched across sources: {len(all_articles)}")

    # Deduplicate based on URL primarily, then title if URL is missing
    ...
    for article in all_articles:
         ...
         # Prioritize URL for deduplication
         if url and url not in seen_urls:
             unique_articles.append(article); seen_urls.add(url);
             if title_lower: seen_titles_lower.add(title_lower) # Add title to prevent duplicate if URL was missing later
         # Fallback to title if URL is missing
         elif not url and title_lower and title_lower not in seen_titles_lower:
             unique_articles.append(article); seen_titles_lower.add(title_lower)

    logging.info(f"[News] Articles after deduplication: {len(unique_articles)}")

    # Sort by date (descending) and take the most recent N for analysis
    def get_sort_key(article):
        ...
        return datetime.min.replace(tzinfo=timezone.utc) # Put articles without date at the end

    unique_articles.sort(key=get_sort_key, reverse=True)
    articles_to_analyze = unique_articles[:NEWS_MAX_ARTICLES_DISPLAY]
    ...
    if not articles_to_analyze:
        ...
        counts = {'positive': 0, 'negative': 0, 'neutral': 0, 'total': 0, 'avg_score': 0.0}; return summary_str, counts

    for article in articles_to_analyze:
        title = article.get('title', ''); description = article.get('description', '');
        # Use description if available and seems substantial, otherwise just title
        text_to_analyze = f"{title}. {description}" if description and len(description) > 20 else title
    ...
    total_analyzed = len(articles_to_analyze)
    avg_score = np.mean(compound_scores) if compound_scores else 0.0

    # Determine overall bias using a simple heuristic
    if positive_count > negative_count + neutral_count * 0.5: overall_bias = "Positive Bias ✅" # Need significantly more positive
    elif negative_count > positive_count + neutral_count * 0.5: overall_bias = "Negative Bias ❌" # Need significantly more negative
    ...
    logging.info(f"[News] Result: P={positive_count}, N={negative_count}, Neut={neutral_count}, AvgScore={avg_score:.3f}")
    return summary_str, counts
...
def get_financial_data(ticker_obj, statement_type: str, periods: int = 2):
    """Safely gets specified financial statement from yfinance ticker object."""
    try:
...
def safe_get(series: pd.Series, key: str, default=None):
    """Safely retrieves and converts a value from a Pandas Series."""
...
def get_stock_beta(ticker_info: dict):
    """Attempts to retrieve Beta from yfinance info dict."""
...
def normalize_score(value, range_min, range_max, higher_is_riskier=True, is_log_range=False):
    """Normalizes a value to a 0-100 risk score."""
    ...
    if is_log_range:
        ...
        else: return 0.0 if higher_is_riskier else 100.0 # Treat zero/negative as lowest/highest depending on higher_is_riskier
    # Ensure range_min != range_max to avoid division by zero
    if abs(current_max - current_min) < 1e-9: return 50.0 # Return average score if range is zero
...
@st.cache_data(ttl=3600*6) # Cache F-Score for 6 hours
def calculate_piotroski_f_score(ticker_symbol: str):
    """Calculates Piotroski F-Score using yfinance data. Cachable."""
    logging.info(f"[{ticker_symbol}] Calculating Piotroski F-Score (or cache)...")
    score = 0
    details = {}
    try:
        ticker = yf.Ticker(ticker_symbol)
        income = get_financial_data(ticker, 'income_stmt', 2)
        balance = get_financial_data(ticker, 'balance_sheet', 2)
        cashflow = get_financial_data(ticker, 'cashflow', 2)

        if income is None or balance is None or cashflow is None or \
           income.shape[1] < 2 or balance.shape[1] < 2 or cashflow.shape[1] < 2:
            logging.warning(f"[{ticker_symbol}] Insufficient financials for F-Score.");
            return None, details

        inc_t, inc_tm1 = income.iloc[:, 0], income.iloc[:, 1]
        bal_t, bal_tm1 = balance.iloc[:, 0], balance.iloc[:, 1]
        cf_t, cf_tm1 = cashflow.iloc[:, 0], cashflow.iloc[:, 1]

        # Profitability
        net_income_t = safe_get(inc_t, 'Net Income', 0)
        details['NI > 0']=(net_income_t is not None and net_income_t > 0)
        score += details['NI > 0']

        op_cashflow_t=safe_get(cf_t, 'Operating Cash Flow', safe_get(cf_t, 'Cash Flow From Continuing Operating Activities', 0))
        details['OCF > 0']=(op_cashflow_t is not None and op_cashflow_t > 0)
        score += details['OCF > 0']

        assets_t=safe_get(bal_t, 'Total Assets', 0)
        assets_tm1=safe_get(bal_tm1, 'Total Assets', 0)
        avg_assets_t=(assets_t + assets_tm1) / 2 if assets_t!=0 and assets_tm1!=0 else (assets_t or assets_tm1 or 0) # Handle potential zeros more robustly
        avg_assets_tm1 = assets_tm1 # Usually just the prior period's assets
        roa_check=False
        if avg_assets_t != 0 and avg_assets_tm1 != 0:
            net_income_tm1=safe_get(inc_tm1, 'Net Income', 0)
            roa_t=net_income_t/avg_assets_t if avg_assets_t != 0 else 0 # Prevent division by zero
            roa_tm1=net_income_tm1/avg_assets_tm1 if avg_assets_tm1 != 0 else 0 # Prevent division by zero
            roa_check=(roa_t > roa_tm1)
        else:
             logging.warning(f"[{ticker_symbol}] Cannot calculate ROA change (F-Score). Assets: t={assets_t}, tm1={assets_tm1}")
        details['Delta ROA > 0']=roa_check
        score += details['Delta ROA > 0']

        details['OCF > NI']=(op_cashflow_t > net_income_t) if op_cashflow_t is not None and net_income_t is not None else False
        score += details['OCF > NI']

        # Leverage, Liquidity, Source of Funds
        debt_lt_t=safe_get(bal_t, 'Long Term Debt And Capital Lease Obligation', safe_get(bal_t, 'Long Term Debt', 0))
        debt_lt_tm1=safe_get(bal_tm1, 'Long Term Debt And Capital Lease Obligation', safe_get(bal_tm1, 'Long Term Debt', 0))
        leverage_check=False
        if avg_assets_t != 0 and avg_assets_tm1 != 0:
            leverage_t=debt_lt_t/avg_assets_t if debt_lt_t is not None and avg_assets_t != 0 else np.inf
            leverage_tm1=debt_lt_tm1/avg_assets_tm1 if debt_lt_tm1 is not None and avg_assets_tm1 != 0 else np.inf
            leverage_check=(leverage_t < leverage_tm1)
        else:
             logging.warning(f"[{ticker_symbol}] Cannot calculate Leverage change (F-Score). Assets: t={assets_t}, tm1={assets_tm1}")
        details['Delta Leverage < 0']=leverage_check
        score += details['Delta Leverage < 0']

        current_assets_t=safe_get(bal_t, 'Current Assets', 0)
        current_liab_t=safe_get(bal_t, 'Current Liabilities', 0)
        current_ratio_t=current_assets_t/current_liab_t if current_liab_t!=0 and current_assets_t is not None else 0
        current_assets_tm1=safe_get(bal_tm1, 'Current Assets', 0)
        current_liab_tm1=safe_get(bal_tm1, 'Current Liabilities', 0)
        current_ratio_tm1=current_assets_tm1/current_liab_tm1 if current_liab_tm1!=0 and current_assets_tm1 is not None else 0
        details['Delta Current Ratio > 0']=(current_ratio_t > current_ratio_tm1)
        score += details['Delta Current Ratio > 0']
        ...
        shares_check=None
        if shares_t is not None and shares_tm1 is not None and shares_tm1 > 0: # Added check for shares_tm1 > 0
            shares_check=(shares_t <= shares_tm1 * 1.01) # Allowing slight increase (e.g., stock options)
            details['Shares Issued Not Increased']=shares_check
        else:
            logging.warning(f"[{ticker_symbol}] Shares Issued not found or zero, using equity proxy for F-Score.")
            equity_t=safe_get(bal_t, 'Stockholders Equity', 0)
            equity_tm1=safe_get(bal_tm1, 'Stockholders Equity', 0)
            ...
            if equity_growth_non_re is not None:
                 if equity_tm1 is not None and equity_tm1 != 0:
                    shares_check=(equity_growth_non_re < equity_tm1 * 0.02)
                 ...
                 details['Shares Issued Not Increased (Equity Proxy)'] = False
        score += shares_check if shares_check is not None else 0

        # Operating Efficiency
        gross_profit_t=safe_get(inc_t, 'Gross Profit', 0)
        revenue_t=safe_get(inc_t, 'Total Revenue', safe_get(inc_t,'Operating Revenue', 0))
        gross_margin_t=gross_profit_t/revenue_t if revenue_t!=0 and gross_profit_t is not None else 0
        gross_profit_tm1=safe_get(inc_tm1, 'Gross Profit', 0)
        revenue_tm1=safe_get(inc_tm1, 'Total Revenue', safe_get(inc_tm1,'Operating Revenue', 0))
        gross_margin_tm1=gross_profit_tm1/revenue_tm1 if revenue_tm1!=0 and gross_profit_tm1 is not None else 0
        details['Delta Gross Margin > 0']=(gross_margin_t > gross_margin_tm1)
        score += details['Delta Gross Margin > 0']

        turnover_check=False
        if avg_assets_t != 0 and avg_assets_tm1 != 0:
            asset_turnover_t=revenue_t/avg_assets_t if revenue_t is not None and avg_assets_t != 0 else 0
            asset_turnover_tm1=revenue_tm1/avg_assets_tm1 if revenue_tm1 is not None and avg_assets_tm1 != 0 else 0
            turnover_check=(asset_turnover_t > asset_turnover_tm1)
        else:
             logging.warning(f"[{ticker_symbol}] Cannot calculate Asset Turnover change (F-Score). Assets: t={assets_t}, tm1={assets_tm1}")
        details['Delta Asset Turnover > 0']=turnover_check
        score += details['Delta Asset Turnover > 0']

        logging.info(f"[{ticker_symbol}] Piotroski F-Score calculated: {score}/9"); logging.debug(f"[{ticker_symbol}] F-Score Details: {details}");
        return score, details
    except Exception as e:
        logging.error(f"[{ticker_symbol}] General F-Score error: {e}", exc_info=True);
        logging.debug(traceback.format_exc());
        return None, details

# --- Risk Score Functions ---
def owa_cvar(T, alpha=0.01):
    """Calculates OWA weights for CVaR."""
    T_ = int(T);
...
def covar_matrix(X, method="hist", d=0.94, alpha=0.1, bWidth=0.01, detone=False, mkt_comp=1, threshold=0.5):
    """Calculates the covariance matrix using various methods, handling non-PSD cases."""
    ...
    try:
        if method == "hist": cov = np.cov(X.to_numpy(), rowvar=False)
            ...
            S=np.cov(X.to_numpy(), rowvar=False); R=np.corrcoef(X.to_numpy(), rowvar=False); D=np.sqrt(np.clip((1-R)/2, a_min=0.0, a_max=1.0)); np.fill_diagonal(D, 0); D=(D + D.T)/2; Sim=1 - D**2
            (_, _, separators, cliques, _) = db.PMFG_T2s(Sim, nargout=4); cov = db.j_LoGo(S, separators, cliques); cov = np.linalg.inv(cov)
            ...
            cov_hist=np.cov(X.to_numpy(), rowvar=False);
            T, N = X.shape; q = T / N;
            cov=af.denoiseCov(cov_hist, q, kind=method, bWidth=bWidth, detone=detone, mkt_comp=int(mkt_comp), alpha=alpha)
        elif method == "gerber1":
...
def calculate_owa_risk(returns: pd.Series, owa_weights_func, **kwargs):
    """Calculates risk using Ordered Weighted Averaging weights."""
    ...
    if T < 2: return None # Need at least 2 returns for OWA

    try:
        sorted_returns = np.sort(returns.values)
        owa_weights = owa_weights_func(T, **kwargs)
        ...
        # Ensure both arrays are flat and dot product works
...
@st.cache_data(ttl=600) # Cache full risk score calculation for 10 mins
def calculate_dynamic_risk_score(ticker_symbol: str, hist_df: pd.DataFrame, weights: dict = None, cov_method: str = 'hist', vol_window: int = 90):
    """Calculates dynamic risk score (0-100) for a single stock using pre-fetched history."""
    try:
        logging.info(f"[{ticker_symbol}] Risk score func: Using pre-fetched history. Fetching yfinance info...")
        ticker = yf.Ticker(ticker_symbol);
        try: info = ticker.info;
        except Exception as info_err: logging.warning(f"[{ticker_symbol}] Risk score: Info fetch failed: {info_err}."); info = None

        # Risk calculation needs the full history DataFrame
        if hist_df is None or hist_df.empty or 'Close' not in hist_df.columns:
             logging.error(f"[{ticker_symbol}] FAILED Risk Score: Invalid/Empty FULL history DataFrame provided.");
             return None, {}, 0.0
        ...
        returns_1y = hist_df['Close'].pct_change().dropna() # Calculate returns from the close series
        logging.info(f"[{ticker_symbol}] Risk score: Valid return rows for risk: {len(returns_1y)}")

        min_returns_needed_advanced_risk = max(vol_window, int(np.ceil(1 / CVAR_ALPHA)) + 5);
        ...
        last_price = hist_df['Close'].iloc[-1] if not hist_df.empty else info.get('currentPrice', info.get('regularMarketPrice'));
        vix_value = get_vix_cached()
    ...
    # (Volatility, SemiDev, MarketCap, Liquidity, PE/PS, SMA, Beta, Piotroski, VIX, CVaR, CDaR, MDD, GMD)
    # ... [Code for calculating each factor remains the same as previous version] ...
    ...
    # --- ... [Repeat for all other factors: SemiDev, MarketCap, etc.] ... ---

    # --- [Semi Deviation Example] ---
    semi_dev = None; semi_dev_score = None
    if len(returns_1y) >= 2 and current_weights.get('semi_deviation', 0) > 1e-6: # Check if weight is non-zero
    ...
    # --- [MDD Example] ---
    mdd_score = None
    if not hist_df.empty: # MDD needs the price series from the history df
    ...
    # --- Final Score Calculation (As before) ---
    ...
    if total_valid_original_weight < 0.01 or valid_scores_count == 0:
        logging.error(f"[{ticker_symbol}] FAILED Risk Score: Low valid factor weight ({total_valid_original_weight:.3f}).")
        return None, intermediate_scores, total_valid_original_weight
    ...
    for factor, score in intermediate_scores.items():
        if score is not None and factor in renormalized_weights:
            renormalized_weight = renormalized_weights[factor]; final_score += score * renormalized_weight; logging.debug(f"[{ticker_symbol}] Score contribution: '{factor}', Score={score:.2f}, RenormW={renormalized_weight:.3f}, Add={score * renormalized_weight:.2f}")
    ...
    if vol_score_val is not None and cvar_score_val is not None and vol_score_val > 95 and cvar_score_val > 95:
        boost = 5.0; final_score += boost; final_score = min(100.0, final_score); logging.info(f"[{ticker_symbol}] Extreme Volatility + Tail Risk Boost (+{boost:.1f})")
...
# --- Momentum Indicators Function REMOVED ---
# def calculate_momentum_indicators(...) REMOVED

# --- Technical Strategy Definitions (Remain the same) ---
def crossed_above(s1, s2):
    try:
        s1, s2 = pd.to_numeric(s1, errors='coerce'), pd.to_numeric(s2, errors='coerce')
        crossed = (s1.shift(1) <= s2.shift(1)) & (s1 > s2)
        return crossed.fillna(False)
    except Exception as e:
        logging.error(f"Crossed above Error: {e}");
        return pd.Series(False, index=s1.index) if hasattr(s1, 'index') else pd.Series(False)
...
def crossed_below(s1, s2):
    try:
        ...
        logging.error(f"Crossed below Error: {e}");
        return pd.Series(False, index=s1.index) if hasattr(s1, 'index') else pd.Series(False)

def strategy_golden_cross_rsi(df, short_ema=50, long_ema=200, rsi_period=14, rsi_level=50):
    try:
        if df is None or df.empty or "Close" not in df.columns or len(df) < max(short_ema, long_ema, rsi_period): return None
        df_copy = df.copy();
        df_copy["EMA_short"] = ta.ema(df_copy["Close"], length=short_ema);
        df_copy["EMA_long"] = ta.ema(df_copy["Close"], length=long_ema);
        df_copy["RSI"] = ta.rsi(df_copy["Close"], length=rsi_period);
        df_copy.dropna(subset=["EMA_short", "EMA_long", "RSI"], inplace=True);
        if df_copy.empty: return None
        entry = crossed_above(df_copy["EMA_short"], df_copy["EMA_long"]) & (df_copy["RSI"] > rsi_level); df_copy['GC_RSI_Entry'] = entry
        exit_ = crossed_below(df_copy["EMA_short"], df_copy["EMA_long"]); df_copy['GC_RSI_Exit'] = exit_
        return df_copy
    except Exception as e: logging.error(f"GC RSI Strategy Error: {e}"); return None

def strategy_macd_rsi_oversold(df, fast=12, slow=26, signal_p=9, rsi_period=14, rsi_oversold=30):
    try:
        if df is None or df.empty or "Close" not in df.columns or len(df) < max(fast, slow, signal_p, rsi_period): return None
        df_copy = df.copy();
        macd_data = ta.macd(df_copy["Close"], fast=fast, slow=slow, signal=signal_p);
        rsi_data = ta.rsi(df_copy["Close"], length=rsi_period);
        if macd_data is None or macd_data.empty or rsi_data is None or rsi_data.empty: return None
        df_copy["MACD"] = macd_data.get(f"MACD_{fast}_{slow}_{signal_p}");
        df_copy["MACDs"] = macd_data.get(f"MACDs_{fast}_{slow}_{signal_p}");
        df_copy["RSI"] = rsi_data;
        df_copy.dropna(subset=["MACD", "MACDs", "RSI"], inplace=True);
        if df_copy.empty: return None
        entry = crossed_above(df_copy["RSI"], rsi_oversold) & (df_copy["MACD"] > df_copy["MACDs"]); df_copy['MACD_RSI_OS_Entry'] = entry
        exit_ = crossed_below(df_copy["MACD"], df_copy["MACDs"]); df_copy['MACD_RSI_OS_Exit'] = exit_
        return df_copy
    except Exception as e: logging.error(f"MACD RSI OS Strategy Error: {e}"); return None

def strategy_macd_bullish_adx(df, fast=12, slow=26, signal_p=9, adx_period=14, adx_level=25):
    try:
        if df is None or df.empty or not all(c in df.columns for c in ["High", "Low", "Close"]) or len(df) < max(fast, slow, signal_p, adx_period): return None
        df_copy = df.copy();
        macd_data = ta.macd(df_copy["Close"], fast=fast, slow=slow, signal=signal_p);
        adx_data = ta.adx(df_copy["High"], df_copy["Low"], df_copy["Close"], length=adx_period);
        if macd_data is None or macd_data.empty or adx_data is None or adx_data.empty: return None
        df_copy["MACD"] = macd_data.get(f"MACD_{fast}_{slow}_{signal_p}");
        df_copy["MACDs"] = macd_data.get(f"MACDs_{fast}_{slow}_{signal_p}");
        df_copy["ADX"] = adx_data.get(f"ADX_{adx_period}");
        df_copy.dropna(subset=["MACD", "MACDs", "ADX"], inplace=True);
        if df_copy.empty: return None
        entry = crossed_above(df_copy["MACD"], df_copy["MACDs"]) & (df_copy["ADX"] > adx_level); df_copy['MACD_ADX_Entry'] = entry
        exit_ = crossed_below(df_copy["MACD"], df_copy["MACDs"]); df_copy['MACD_ADX_Exit'] = exit_
        return df_copy
    except Exception as e: logging.error(f"MACD ADX Strategy Error: {e}"); return None

def strategy_adx_heikin_ashi(df, adx_period=14, adx_level=25):
    try:
        if df is None or df.empty or not all(c in df.columns for c in ["Open", "High", "Low", "Close"]) or len(df) < adx_period + 1: return None # HA needs 1 prior row
        df_copy = df.copy();
        adx_data = ta.adx(df_copy["High"], df_copy["Low"], df_copy["Close"], length=adx_period);
        ha_df = ta.ha(df_copy["Open"], df_copy["High"], df_copy["Low"], df_copy["Close"]);
        if adx_data is None or adx_data.empty or ha_df is None or ha_df.empty: return None
        df_copy["ADX"] = adx_data.get(f"ADX_{adx_period}");
        df_copy['HA_open'] = ha_df['HA_open']; df_copy['HA_close'] = ha_df['HA_close'];
        df_copy.dropna(subset=["ADX", "HA_open", "HA_close"], inplace=True);
        if df_copy.empty: return None
        entry = (df_copy["ADX"] > adx_level) & (df_copy['HA_close'] > df_copy['HA_open']) & (df_copy['HA_close'].shift(1) <= df_copy['HA_open'].shift(1)).fillna(False); df_copy['ADX_HA_Entry'] = entry
        exit_ = df_copy['HA_close'] < df_copy['HA_open']; df_copy['ADX_HA_Exit'] = exit_
        return df_copy
    except Exception as e: logging.error(f"ADX HA Strategy Error: {e}"); return None

def strategy_psar_rsi(df, initial_af=0.02, max_af=0.2, rsi_period=14, rsi_level=50):
    try:
        if df is None or df.empty or not all(c in df.columns for c in ["High", "Low", "Close"]) or len(df) < max(rsi_period, 10): return None # PSAR needs some data
        df_copy = df.copy();
        psar = ta.psar(df_copy["High"], df_copy["Low"], close=df_copy["Close"], af0=initial_af, af=initial_af, afmax=max_af);
        if psar is None or psar.empty: logging.warning("PSAR calc None/empty."); return None
        # Handle potential column name variations from pandas_ta
        af_str = f"{initial_af}".replace('.', ''); max_af_str = f"{max_af}".replace('.', '')
        psarl_col = next((col for col in psar.columns if col.startswith(f'PSARl_') and col.endswith(f'_{af_str}_{max_af_str}')), f"PSARl_{initial_af}_{max_af}")
        psars_col = next((col for col in psar.columns if col.startswith(f'PSARs_') and col.endswith(f'_{af_str}_{max_af_str}')), f"PSARs_{initial_af}_{max_af}")
        psarr_col = next((col for col in psar.columns if col.startswith(f'PSARr_') and col.endswith(f'_{af_str}_{max_af_str}')), f"PSARr_{initial_af}_{max_af}")

        if not all(col in psar.columns for col in [psarl_col, psars_col, psarr_col]):
             logging.error(f"PSAR cols not found: {psar.columns}. Tried: {psarl_col}, {psars_col}, {psarr_col}"); return None

        df_copy["PSARl"] = psar.get(psarl_col); df_copy["PSARs"] = psar.get(psars_col); df_copy["PSARr"] = psar.get(psarr_col);
        df_copy["RSI"] = ta.rsi(df_copy["Close"], length=rsi_period);
        df_copy.dropna(subset=["PSARl", "PSARs", "RSI", "PSARr"], inplace=True);
        if df_copy.empty: return None
        entry = (df_copy["PSARr"] == 1) & (df_copy["RSI"] > rsi_level); df_copy['PSAR_RSI_Entry'] = entry
        exit_ = (df_copy["PSARr"] == -1); df_copy['PSAR_RSI_Exit'] = exit_
        return df_copy
    except Exception as e: logging.error(f"PSAR RSI Strategy Error: {e}"); return None


# --- Strategy Scanning Function (Uses pre-fetched yfinance Data) ---
@st.cache_data(ttl=600) # Cache the calculation result
def scan_strategies(hist_df: pd.DataFrame, ticker: str):
    """Runs multiple technical strategies on the provided pre-fetched DataFrame."""
    # Needs the full history DataFrame
    min_rows_needed_scan = 250 # Needs enough data for 200 SMA, etc.
    if hist_df is None or hist_df.empty or len(hist_df) < min_rows_needed_scan:
        logging.warning(f"Not enough pre-fetched yfinance data ({len(hist_df) if hist_df is not None else 0}) for scan ({ticker}). Needed: {min_rows_needed_scan}")
        return f"Not enough historical data provided for technical strategy scan ({ticker}). Requires approx. 1 year of daily data.", False

    try:
        logging.info(f"Starting yfinance technical strategy scan for {ticker} using PRE-FETCHED data ({len(hist_df)} rows)")
        strategies = {
            'GC_RSI': strategy_golden_cross_rsi,
            'MACD_ADX': strategy_macd_bullish_adx,
            'MACD_RSI_OS': strategy_macd_rsi_oversold,
            'ADX_HA': strategy_adx_heikin_ashi,
            'PSAR_RSI': strategy_psar_rsi
        }
        entries, exits = [], [];
        last_valid_index = hist_df.index[-1];
        combined_df = hist_df.copy() # Start with the original full history

        for name, func in strategies.items():
            try:
                res_df = func(hist_df.copy()) # Pass a clean copy of the original df
                if res_df is not None and not res_df.empty:
                    entry_col, exit_col = f"{name}_Entry", f"{name}_Exit"
                    combined_df[entry_col] = res_df[entry_col].reindex(combined_df.index).fillna(False).astype(bool)
                    combined_df[exit_col] = res_df[exit_col].reindex(combined_df.index).fillna(False).astype(bool)
                else:
                    combined_df[f"{name}_Entry"] = False
                    combined_df[f"{name}_Exit"] = False
                    logging.warning(f"Strategy {name} for {ticker} returned no valid signals.")
            except Exception as strategy_err:
                 logging.error(f"Error running strategy {name} for {ticker}: {strategy_err}", exc_info=False)
                 combined_df[f"{name}_Entry"] = False
                 combined_df[f"{name}_Exit"] = False

        if last_valid_index in combined_df.index:
             last_day_signals = combined_df.loc[last_valid_index]
             for name in strategies.keys():
                 entry_col, exit_col = f"{name}_Entry", f"{name}_Exit"
                 if entry_col in last_day_signals and pd.notna(last_day_signals.get(entry_col)) and last_day_signals.get(entry_col) is True: entries.append(entry_col)
                 if exit_col in last_day_signals and pd.notna(last_day_signals.get(exit_col)) and last_day_signals.get(exit_col) is True: exits.append(exit_col)
        else:
            logging.warning(f"Last index {last_valid_index} from original df not found in strategy result df for {ticker}.")
            if not combined_df.empty: # Fallback check
                last_valid_index = combined_df.index[-1]
                logging.warning(f"Checking against last index of results df: {last_valid_index}")
                if last_valid_index in combined_df.index:
                    last_day_signals = combined_df.loc[last_valid_index]
                    for name in strategies.keys():
                        entry_col, exit_col = f"{name}_Entry", f"{name}_Exit"
        ...
        has_signals = bool(entries or exits)
        if has_signals:
            summary = f"Latest Technical Strategy Signals for {ticker} (Yahoo Finance Daily Data):\n"
            if entries: summary += f"  🚀 Entry: {', '.join(entries)}\n"
            if exits: summary += f"  🛑 Exit: {', '.join(exits)}\n"; summary += "(Note: Based on historical daily data, not predictions.)"
        else:
             if not combined_df.empty and any(f"{name}_Entry" in combined_df.columns for name in strategies.keys()):
                summary = f"No active technical entry/exit signals found for {ticker} on last daily bar (Yahoo Finance data)."
             else:
                summary = f"Technical strategy scan failed or returned no data for {ticker}."

        logging.info(f"yfinance technical scan complete for {ticker}. Signals found: {has_signals}. Summary: {summary}");
        return summary, has_signals
    except Exception as e:
        logging.error(f"Overall yfinance strategy scan error for {ticker}: {e}", exc_info=True);
        return f"Error during technical strategy scan for {ticker}.", False


# --- LLM Prompt Formatting ---
def format_stock_data_for_prompt(data):
    """Formats the fetched Yahoo Finance summary data into a string for the LLM prompt."""
    if not data or not data.get('ticker'): return "No current Yahoo Finance summary data found for this ticker."
    ticker = data['ticker']
    lines = [ f"Context - Summary data for {ticker} ({data.get('companyName', 'N/A')}) from Yahoo Finance:",
              f"- Current/Last Price: {format_val(data.get('priceForDisplay'), '$', prec=2)}",
              f"- Market Cap: {format_val(data.get('marketCap'), '$', prec=2)}", # Changed prec to 2
              f"- P/E (Trailing): {format_val(data.get('trailingPE'), prec=2)}",
              f"- P/E (Forward): {format_val(data.get('forwardPE'), prec=2)}",
              f"- Dividend Yield: {format_val(data.get('dividendYield', 0) * 100 if data.get('dividendYield') is not None else 0, suffix='%', prec=2)}",
              f"- 50d SMA: {format_val(data.get('sma50'), '$', prec=2)}",
              f"- 200d SMA: {format_val(data.get('sma200'), '$', prec=2)}",
              f"- Beta: {format_val(data.get('beta'), prec=2)}",
              f"- Day Range: {format_val(data.get('dayLow'), '$', prec=2)} - {format_val(data.get('dayHigh'), '$', prec=2)}",
              f"- 52 Week Range: {format_val(data.get('fiftyTwoWeekLow'), '$', prec=2)} - {format_val(data.get('fiftyTwoWeekHigh'), '$', prec=2)}",
              f"- Volume: {format_val(data.get('volume'), prec=0)}", # Volume doesn't need decimals
             ]
    rec_key = data.get('recommendationKey')
    num_opinions = data.get('numberOfAnalystOpinions')
    mean_target = data.get('targetMeanPrice')
    low_target = data.get('targetLowPrice')
    high_target = data.get('targetHighPrice')
    mapped_rec = map_recommendation_key_to_english(rec_key)
    analyst_line = "- Analyst Consensus (Aggregated Yahoo): Not Available"

    if mapped_rec not in ["Not Available", "N/A"] and mean_target is not None:
        analyst_count_str = f"{num_opinions} analysts" if num_opinions and isinstance(num_opinions, int) and num_opinions > 0 else "count unavailable";
        ...
        range_str = f", ranging from {target_low_str} to {target_high_str}" if target_low_str != "Not Available" and target_high_str != "Not Available" else "";
        analyst_line = f"- Analyst Consensus (Aggregated Yahoo): Based on {analyst_count_str}, recommendation: {mapped_rec}, avg target: {target_mean_str}{range_str}."
    elif mapped_rec not in ["Not Available", "N/A"]:
        analyst_count_str = f"{num_opinions} analysts" if num_opinions and isinstance(num_opinions, int) and num_opinions > 0 else "count unavailable";
        analyst_line = f"- Analyst Consensus (Aggregated Yahoo): Based on {analyst_count_str}, recommendation: {mapped_rec} (target price N/A)."
    ...
    # Filter out lines explicitly stating "Not Available" or "N/A" unless it's the analyst line where count might be N/A
    filtered_lines = [ln for ln in lines if not (ln.strip().endswith(": Not Available") or ln.strip().endswith(": N/A") ) or ln.startswith("- Analyst Consensus")];

    valid_data_points = sum(1 for ln in filtered_lines[1:] if "Not Available" not in ln and "N/A" not in ln and ln != analyst_line)
...
def format_val(v, prefix="", suffix="", prec=2):
    """Formats numeric values nicely, including T/B/M suffixes, respecting precision."""
    if v is None or pd.isna(v) or str(v).lower() == 'n/a' or str(v).strip() == '': return "Not Available"
    try:
        v_float = float(v)
        # Handle large numbers only if prec > 0 or not specified for integers
        ...
        else: # Handle integers specifically if prec is 0
...
def build_sp500_ticker_map(cache_duration_hours=24, force_refresh=False):
    ...
    else: logging.error("ERROR: S&P 500 Ticker map is None."); return None # Return None if map failed to build
    return ticker_map
...
def build_nasdaq100_ticker_map(cache_duration_hours=24, force_refresh=False):
            ...
            try:
                ...
                if len(tables) <= table_index:
                    ...
                    for i, df in enumerate(tables):
                        cols_lower = {str(col).lower() for col in df.columns}; has_ticker = any(t in cols_lower for t in ['ticker', 'symbol']); has_name = any(n in cols_lower for n in ['company', 'name', 'security'])
    ...
    else: logging.error("ERROR: Nasdaq 100 Ticker map is None."); return None # Return None if map failed to build
    return ticker_map
...
def get_ticker_from_combined_map(query, combined_map):
    ...
    # Add specific common name cleanups not covered by regex
    if query_lower == "coca cola": query_cleaned = "coca-cola"
    elif query_lower == "johnson and johnson": query_cleaned = "johnson & johnson"
    elif query_lower == "google": query_cleaned = "alphabet" # Map google to alphabet for map lookup
    ...
    # Also check for common variations like "google inc" if "google" didn't work initially
...
@st.cache_resource(ttl=3600 * 12) # Cache combined map resource for 12 hours
def load_combined_ticker_map():
    logging.info("\n" + "="*30 + " Building/Loading Combined Ticker Map " + "="*30)
    logging.info("--- Processing S&P 500 ---"); sp500_map = build_sp500_ticker_map();
    ...
    logging.info("\n--- Merging Maps ---"); combined_tickers = sp500_map.copy(); combined_tickers.update(nasdaq100_map)
    logging.info(f"Total entries in combined map: {len(combined_tickers)}"); logging.info(" Combined Ticker Map Ready " + "="*30 + "\n"); return combined_tickers

# --- Ticker/Company Lookup Function ---
@st.cache_data(ttl=3600) # Cache lookup results for 1 hour
def lookup_ticker_by_company_name(query):
    """Attempts to find a valid equity/ETF ticker symbol based on user query (ticker or company name)."""
    ...
    if is_potential_ticker:
        ...
        try:
            ...
            if info and info.get('symbol', '').upper() == direct_ticker_attempt and info.get('quoteType') in ['EQUITY', 'ETF'] and info.get('regularMarketPrice') is not None:
                logging.info(f"Step 1 SUCCESS: Direct yf found {info.get('quoteType')}: {direct_ticker_attempt}"); return direct_ticker_attempt
            else:
                 ...
                 if not hist.empty:
                     if info and info.get('quoteType') and info.get('quoteType') not in ['EQUITY', 'ETF']:
                         ...
                         logging.info(f"Step 1 SUCCESS (via history): Direct yf '{direct_ticker_attempt}' confirmed (likely EQUITY/ETF)."); return direct_ticker_attempt
    ...
    try:
        ...
        for item in quotes:
            ...
            # Allow EQUITY or ETF, filter out obvious non-stocks
            if (quote_type not in ["EQUITY", "ETF"] or not symbol or '^' in symbol or any(ft in quote_type for ft in ['FUTURE', 'INDEX', 'CURRENCY', 'OPTION', 'MUTUALFUND']) or item.get("typeDisp") in ['Index', 'Currency', 'Futures', 'Option', 'Mutual Fund']):
...
# --- System Prompt (UPDATED for ETS FORECASTING and REMOVED PREDICTIONS) ---
SYSTEM_PROMPT = f"""
...
3.  Technical Strategy Signals (**calculated using Yahoo Finance daily data**).
4.  A recent News Sentiment summary (**aggregated from NewsAPI, FMP, and RSS feeds over the last {NEWS_DAYS_BACK} days, analyzed using VADER**).
5.  A short-term price forecast (**calculated using an ETS model on historical Yahoo Finance daily data**).
...
4.  **Prioritize using the data provided in the context.** Integrate data from **Yahoo Finance** for fundamentals and analyst views. Use **Yahoo Finance daily data** for technical strategy signals. Incorporate the provided **Model** Risk Score. Use the **Multi-Source News Sentiment (VADER analysis)** summary. Use the **ETS Forecast** data.
5.  If presented with current data, incorporate it naturally. State the source for each piece of data clearly: **Yahoo Finance** for fundamentals/analyst views, **Yahoo Finance daily data** for technicals, **Multi-Source News/VADER** for news sentiment, **ETS Model (Calculated)** for the forecast, and **'Model'** for the risk score.
6.  If asked about a specific ticker, **structure your answer logically:**
    ...
    e. **Technical Strategy Scan Summary:** Mention specific signals (e.g., 'GC_RSI_Entry') if listed in the context and briefly explain their general meaning. State this is based on historical daily data (**Yahoo Finance**) and not predictive. If no signals or scan failed, state that clearly.
    f. **Recent News Sentiment Summary:** Present the news sentiment summary **exactly as provided in the context (sourced from Multi-Source News/VADER over {NEWS_DAYS_BACK} days).** Mention the counts (positive, negative, neutral) and the overall bias if available. State this reflects *recent* news articles.
    g. **ETS Price Forecast:** Present the calculated forecast **exactly as provided in the context.** Mention the forecast period (e.g., "next 7 days"), the model used (e.g., "ETS Model"), and include the evaluation summary (RMSE/MAPE) if available. State this is a model-based forecast and not a guarantee. If the forecast is 'Unavailable' or there was an error, state that clearly.
...
11. If a user asks specifically to "scan" a ticker, focus the response on the technical analysis summary (point 6e) using **Yahoo Finance daily data** results from the context.
...
""" # Removed SharePredictions explanation
...
# --- UI Elements ---
# Updated Title
st.markdown('<h1 style="text-align: left;">📈 Financial Chat, Strategy Scanner, Risk, News & Forecasting</h1>', unsafe_allow_html=True)
# Updated description
st.markdown(f'<p style="text-align: left; font-size: small;">Ask about stocks ($AAPL, Microsoft), scan signals ("scan $NVDA"), compare, or discuss finance. Includes Dynamic Risk Score, Technical Strategy Signals (Yahoo Finance daily data), Recent News Sentiment (Multi-Source/{NEWS_DAYS_BACK}d/VADER), and ETS Price Forecasting (Calculated). No charts.</p>', unsafe_allow_html=True)

with st.sidebar:
    st.image("https://streamlit.io/images/brand/streamlit-mark-color.png", width=50)
    st.markdown("## Examples")
    # Updated MENU_OPTIONS - removed Momentum, removed Predictions, added Forecasting
    MENU_OPTIONS = {
        "🔍 Stock Info": ["What's up with $TSLA?", "Tell me about Apple", "Info on Coca-Cola?", "$ESLT.TA details", "Microsoft data?", "3M Company info?"],
        "🤖 Scan Signals": ["Scan $SPY", "Entry signal on $MSFT?", "Scan $NVDA for signals"],
        "🆚 Compare": ["Compare $MSFT to $GOOGL", "Intel vs Nvidia?", "Compare 3M and General Electric"],
        "📊 TA Concepts": ["What is SMA?", "Explain RSI?", "Resistance breakout?", "Candlesticks?", "What are technical indicators?"],
        ...
        "📈 ETS Forecast": ["Forecast $AAPL price", "What's the ETS forecast for $MSFT?", "Price projection for $GOOG?"], # Added Forecast category
    ...
    for category, questions in MENU_OPTIONS.items():
        is_expanded = (category in ["🔍 Stock Info", "⚖️ Risk", "📰 News Sentiment", "📈 ETS Forecast"]) # Expanded forecast category
    ...
    if not NEWS_API_KEY or len(NEWS_API_KEY) < 20:
        st.warning("NewsAPI Key missing/invalid. News sentiment may be limited.", icon="⚠️")
    if not FMP_API_KEY or len(FMP_API_KEY) < 20:
        st.warning("FMP API Key missing/invalid. News sentiment may be limited.", icon="⚠️")
    # Removed SharePredictions warning
...
# Updated input hint
chat_input_value = st.chat_input(f"Ask about stocks ($AAPL, Microsoft), 'scan $XYZ', risk, news ({NEWS_DAYS_BACK}d), forecast, or finance...")
...
if user_input_triggered:
    ...
    with st.chat_message("assistant"):
        ...
        if extracted_tickers:
            ...
            # Command prefixes updated for new news and forecast wording
            command_prefixes = ("scan ", "compare ", "risk ", "risky ", "risk score ", "what is the risk score for ", "news sentiment for ", "recent news sentiment for ", "sentiment analysis for ", "forecast ", "price forecast ", "ets forecast ", "price projection for ")
        ...
        if lookup_query:
            ...
            if validated_ticker:
                ...
                if len(lookup_query) > 1:
                     if is_potential_ticker or len(lookup_query.split()) > 1 or len(lookup_query) > 4:
        ...
        full_history_df = None # Store full df for risk/scan
        strategy_scan_summary = "Technical Strategy Scan: Not performed."
        scan_has_signals = False;
        ...
        ets_forecast_summary = "ETS Price Forecast: Not calculated." # New variable for forecast summary
        ...
        # --- Fetch Data, Scan, Calculate Risk, News & Forecast if ticker identified ---
        if primary_ticker:
            with st.spinner(f"Gathering data & insights for **{primary_ticker}**..."):
                ...
                # 2. Fetch Unified History (Needed for Risk, Scan & Forecast)
                placeholder.markdown(f"⏳ Fetching Yahoo Finance history for **{primary_ticker}**...")
                # Fetch 3 years for potentially better ETS forecasting
                ...
                # 4. Run Strategy Scan (Uses full history df)
                placeholder.markdown(f"⏳ Running Technical Strategy Scan for **{primary_ticker}**...")
                if full_history_df is not None and not full_history_df.empty:
                    strategy_scan_summary, scan_has_signals = scan_strategies(full_history_df, primary_ticker)
                else:
                    strategy_scan_summary = f"Technical strategy scan unavailable for {primary_ticker} (History issue)."; scan_has_signals = False; logging.warning("Strategy scan skipped.")
                ...
                if close_prices_history is not None and not close_prices_history.empty:
                    try:
                        ...
                        if forecast_values is not None:
                            ...
                            if eval_metric_str and "Error" not in eval_metric_str:
                                 forecast_lines.append(f"- {eval_metric_str}") # Add RMSE/MAPE if available
                            ...
                            logging.info(f"ETS Forecast generated successfully for {primary_ticker}.")
                        else:
                        ...
                        logging.error(f"Error calling ETS forecast function: {forecast_err}", exc_info=True)
                        ets_forecast_summary = f"ETS Price Forecast: Error during calculation for {primary_ticker}."
                else:
                    ets_forecast_summary = f"ETS Price Forecast: Unavailable due to missing historical price data for {primary_ticker}."
        ...
        if primary_ticker:
             ...
             # --- Add the Specific Context Block (Updated with Forecast) ---
             context_message_content = ""
             ...
             context_message_content += f"Technical Strategy Scan Summary (Yahoo Finance Daily Data):\n{strategy_scan_summary}\n\n"
             ...
             context_message_content += f"Please prioritize info in context. Follow guidelines (missing data, analyst format, state sources: Yahoo Finance, Multi-Source News/VADER, ETS Model, etc.). Note news covers last {NEWS_DAYS_BACK} days. ETS Forecast is short-term."
        ...
        try:
            ...
            # Display Data Dashboard AFTER response generation (pass news counts dict)
            if primary_ticker and stock_data: # Ensure stock_data exists for dashboard
                 ...
                 display_stock_data_dashboard(
                     ...
                     # No prediction data passed anymore
                 )
            elif primary_ticker:
                 st.warning(f"Could not display data snapshot for {primary_ticker} (Yahoo summary data missing).")

        except AuthenticationError as e: # Catch specific authentication error
            ...
            logging.error(f"OpenAI API Authentication Error: {e}", exc_info=True);
            st.session_state.messages.append({"role": "assistant", "content": error_message})
            st.stop() # Stop execution if authentication fails
        except OpenAIError as e:
            error_message = f"😥 OpenAI API Error: {e}. Try again later.";
            placeholder.error(error_message);
